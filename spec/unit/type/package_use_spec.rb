...
describe Puppet::Type.type(:package_use) do
  ...
  describe "when validating the use property" do
    ...
    it "should reject use flags with a space" do
      expect { described_class.new(:name => "sys-devel/gcc", :use => "open mp") }.to raise_error(ArgumentError)
    end
  ...
  describe "when validating the use property" do
    ...
    it "should properly handle nested arrays" do
      described_class.new(:name => "sys-devel/gcc", :use => ["foo",["bar"]]).property(:use).insync?(["foo","bar"]).should eq true
    end
  end
end
